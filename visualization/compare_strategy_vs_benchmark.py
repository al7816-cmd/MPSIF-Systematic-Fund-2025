...
# ---------------------------------------------------------------------
strategy_df = pd.read_csv(STRATEGY_FILE, parse_dates=["date"])
if "daily_return" in strategy_df.columns:
    strategy_rets = strategy_df.set_index("date")["daily_return"]
else:
    # fallback if already single column
    strategy_rets = strategy_df.squeeze("columns")
    strategy_rets.index = pd.to_datetime(strategy_rets.index)
...
axes[0].text(strategy_cum.index[-250], float(strategy_cum.iloc[-1])*0.9,
             f"Strategy: {float(strategy_cum.iloc[-1]-1):.1%}", color="#2E86AB", fontsize=10, fontweight="bold")
axes[0].text(benchmark_cum.index[-250], float(benchmark_cum.iloc[-1])*0.9,
             f"Benchmark: {float(benchmark_cum.iloc[-1]-1):.1%}", color="#E74C3C", fontsize=10, fontweight="bold")
